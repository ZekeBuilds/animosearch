...
export default function NotFound() {
  return (
    <>
      <Helmet>
        <title>Page Not Found — AnimoSearch</title>
      </Helmet>
      <div className="min-h-screen flex flex-col items-center justify-center bg-[var(--color-sky-bg)] dark:bg-[#0D1F14] text-center px-4 pt-20">
        <div className="relative mb-8">
          <div className="w-24 h-24 rounded-full bg-[var(--color-primary)]/10 dark:bg-[var(--color-primary)]/20 flex items-center justify-center mx-auto">
            <BookOpen size={40} className="text-[var(--color-primary)]" aria-hidden="true" />
          </div>
        ...
        <h2 className="font-display font-semibold text-2xl text-[var(--color-ink)] dark:text-white mb-4">Thesis Not Found</h2>
        <p className="text-[var(--color-ink-muted)] dark:text-white/60 max-w-sm mb-8 leading-relaxed text-sm">
          This page doesn't exist in our repository. Let's get you back to your research.
        </p>
        <div className="flex flex-wrap gap-3 justify-center">
          ...
          <Link to="/theses" className="btn-outline">
            <Search size={16} aria-hidden="true" /> Browse Theses
          </Link>
        </div>
      </div>
    </>
